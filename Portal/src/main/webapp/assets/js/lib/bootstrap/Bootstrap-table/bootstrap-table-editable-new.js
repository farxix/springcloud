(function($){
            ...
            $.each(columnObj,function(i,obj){
                $.each(obj,function(z,col){
                    if(!isNaN(col.fieldIndex) && col.fieldIndex >= 0){
                        ...
                        //that.columns[col.fieldIndex] = col;
    ...
    BootstrapTable.prototype.initBody = function () {
        ...
        $.each(that.columns,function(indx,col){
            if(col.edit && col.edit.type == 'select'){
                col.edit = $.extend({},$.fn.bootstrapSelectForEditor.defaults,col.edit);
                    ...
                    that.$body.find('>tr').each(function(){
                        ...
                        $.each(col.edit.data,function(i,data){
                            if(data[col.edit.valueField] == rowData[col.field]){
                                if(col.edit.displayField){
                                    $td.html(data[col.edit.displayField]);
                                }else{
    ...
    BootstrapTable.prototype.initEdit = function(){
        ...
        $(document).bind('mousedown',function(event){
            ...
            if(!($target.parents().is(that.$body)) && !($target.parents().is($('.datetimepicker')))){
                setTimeout(function () {
                    recover(that);
    ...
    function initTrClick(that,_this){
        ...
        var value = rowData[tdOpt.field];
        $td.html('<div style="margin:0;padding:0;overflow:hidden;border:solid 0px red;height:'+(height)+'px;width:'+(width)+'px;">'
            +'<input type="text" placeholder="'+placeholder+'" value="'+value+'" style="margin-left: 0px; margin-right: 0px; padding-top: 1px; padding-bottom: 1px; width:100%;height:100%;font-size:12px">'
            +'</div>');
        $td.width(width);
        ...
        if(!tdOpt.edit.type || tdOpt.edit.type == 'text'){
            ...
            $input.bootstrapSelectForEditor(tdOpt.edit);
        }
            ...
            $td.html('<div style="margin:0;padding:0;overflow:hidden;border:solid 0px red;height:'+(height)+'px;width:'+(width)+'px;" class="input-group date form_datetime"  data-link-field="dtp_editable_input">'
                +'<input class="form-control" type="text" value="'+dateFtt(tdOpt.edit.format,value)+'">'
                +'<span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span>'
                +'</div>'
                +'<input type="hidden" id="dtp_editable_input" value="'+dateFtt(tdOpt.edit.format,value)+'"/>'
            );
            if(tdOpt.edit.format == 'yyyy/mm/dd'){
    ...
    function recover(that){
        ...
        if(that.prevEditRow != null){
            that.prevEditRow.find('td').closest('td').siblings().html(function(i,html){
                ...
                if(typeof $(this).find('input[type="text"]').bootstrapSelectForEditor('getText') != 'object'){
                    $(this).find('input[type="text"]').bootstrapSelectForEditor('destroy');
                }
