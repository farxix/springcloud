...
(function($){
	 ...
	 /**
	  * 调用入口:$('#order_status1').bootstrapSelectForEditor({});
	  * options 为对象时是属性设置参数，为字符串时，是调用具体方法，
	  * agrs 为调用方法时的参数
	  */
	 $.fn.bootstrapSelectForEditor = function(option,agrs){
		 var value = null;
		 this.each(function () {
             var $this = $(this),
             data = $this.data('bootstrap.select'),
             options = $.extend({},$.fn.bootstrapSelectForEditor.defaults,$this.data(),
             ...
             if (typeof option === 'string') {
            	 ...
            	 value = $.fn.bootstrapSelectForEditor.methods[option](this,data.options,agrs);
            	 if (option === 'destroy') {
             ...
             if (!data) {
            	...
            	//判断标签类型,如果是select标签则转换成input标签
                 ...
                 $this.data('bootstrap.select', (data = new BootstrapSelectForEditor($this,options)));
             }
        });
	 ...
	 */
	$.fn.bootstrapSelectForEditor.defaults = {
			url    : null, //请求路径
			params : {},   //请求参数
	 ...
	 * 控件方法属性
	 */
	$.fn.bootstrapSelectForEditor.methods = {
			/**
	...
	var BootstrapSelectForEditor = function (el,options) {
    ...
    //初始化控件
    BootstrapSelectForEditor.prototype.init = function(){
    	this.createSelect();//创建文本选择框
    	this.initServer();//数据加载，并创建下拉节点
    	this.initEvent();//添加事件
    };
    //向服务器请求数据
    BootstrapSelectForEditor.prototype.initServer = function(){
    	var $this = this;
    	if($this.options.url){
     ...
     * 创建选择框
	 */
    BootstrapSelectForEditor.prototype.createSelect = function(){
    	var $this = this;
    	var $input = $this.$el;
    	...
    	$('<input type="hidden" readonly name="'+name+'" value="'+options.emptyValue+'">').insertAfter($input);
    };

    /**
     * 事件初始化
     */
    BootstrapSelectForEditor.prototype.initEvent = function(){
    	var $this = this;
    	var $input = $this.$el;
     ...
     * 创建下拉列表
	 */
    BootstrapSelectForEditor.prototype.createDownList = function(data){
    	var $this = this;
    	if(data){
    	...
    	$(document).bind('mousedown',function(event){
			...
			if(!($target.parents().is($div)) && !($target.parents().is($this.$el.parent()))){
				$this.hideDownList();
			}
		});
		//鼠标悬停时选中当前行
     ...
     * 选择文本框keyup事件
     */
    BootstrapSelectForEditor.prototype.keyUp = function(event){
    	var $this = this;
     ...
     * 选择文本框keydown事件
     */
	BootstrapSelectForEditor.prototype.keyDown = function(){
		var $this = this;
    	var $contentDownList = $this.$contentDownList;
    	//输入框keydown事件
     ...
     * 重新加载
     */
    BootstrapSelectForEditor.prototype.reload = function(){
    	var $this = this;
    	if($this.options.url){
    		$this.options.onBeforeLoad.call(this,$this.options.params);
        	$.ajax({
     ...
     */
	BootstrapSelectForEditor.prototype.showDownList = function(showMatch){
		var $this = this;
		var $contentDownList = $this.$contentDownList; 
	 ...
	 */
	BootstrapSelectForEditor.prototype.select = function(selItem,keyEvent){
		...
		this.$contentDownList.find('.'+options.selItemCls).each(function(i,v){
			$(this).css('color','#878787');//被选择的项字体设置成黑色
		...
		if(unSelect){
			...
			options.onSelect.call(this,$selItem.data('jsonData')[$this.options.valueField],$selItem.data('jsonData'),$this.$el.closest('tr'));
		}
	};
	/**
	 * 清空下拉节点
	 */
	BootstrapSelectForEditor.prototype.clear = function(){
		var $this = this;
		var itemsHtml = '';
		if($this.options.emptyText != undefined && $this.options.emptyText != null){
			...
			itemsHtml = '<div style="padding-left:17px;width:100%;height:20px;color:#878787">' + $this.options.emptyText + '</div>';
			//每行tr绑定数据，返回给回调函数
	 ...
	 * 根据值获取文本
	 */
	BootstrapSelectForEditor.prototype.getTextForVal = function(value){
		var $this = this;
		var retVal = '';
     ...
     * 获取光标在字符串中的位置
     */
	BootstrapSelectForEditor.prototype.getCursorIndex = function(){
		var $this = this;
		var index = 0;
	 ...
	 * @param value 值
	 */
	function setValues(bootstrapSelectForEditor,value){
		var $this = bootstrapSelectForEditor;
		var options = $this.options;
		var $input = $this.$el;
    ...
    function makeContAndShow(inputDownList,isReload){
		...
		for(var i=0;i<data_.length;i++){
			itemsHtml += '<div style="padding-left:17px;width:100%;height:20px;line-height:20px;color:#878787">' + data_[i][options.textField] + '</div>';
		}
		$this.html(itemsHtml);
