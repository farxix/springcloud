(function ($) {
    ...
    BootstrapTable.prototype.initFixedColumns = function () {
        ...
        if (this.options.rightFixedColumns) {
            this.$rightfixedBody = $([
                '<div class="fixed-table-column" style="position: absolute;right:0px; border-right:0px solid #ddd;z-index:9;">',
                '<table>',
                '<thead></thead>',
                '<tbody></tbody>',
                '</table>',
    ...
    BootstrapTable.prototype.fitHeaderColumns = function () {
        ...
        if (that.options.rightFixedColumns) {
            ...
            // this.$rightfixedBody.width(157).show();
        }
    };
    ...
    BootstrapTable.prototype.fitBodyColumns = function () {
        ...
        if (that.options.rightFixedColumns) {
           if( document.body.scrollWidth > (window.innerWidth || document.documentElement.clientWidth)){
               $fixedBody.hide();
           }
            if (!this.$body.find('> tr[data-index]').length) {
            ...
            this.$body.find('> tr').each(function (i) {
                that.$rightfixedBody.find('tbody tr:eq(' + i + ')').height($(this).height()+1);
            });
