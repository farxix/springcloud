(function($) {
    var methods = {
        ...
        initExpander: function() {
            ...
            //如果不是叶子节点加个标识
            if($(cell).closest('tr').hasClass('tree-parent')){
                $(cell).find('span.treegrid-expander').addClass('treegrid-collapsed').addClass('treegrid-expander-collapsed');
            }
            return $this
        },
        initIndent: function() {
        ...
        onSelectNode: function(callback){
            var $this = $(this);
            $this.find('tbody').undelegate().delegate('tr td:nth-child(1)','click',function(e){
                e.stopPropagation();
                var $this = $(this).closest('tr');
                var viewOid  = $this.treegrid('getNodeId');
                if($this.treegrid('isLeaf')){
                    $this.find('td').eq(0).find('span.treegrid-expander').after('<i class="fa fa-spinner fa-pulse"></i>');
                    loadChilds(viewOid,$this,callback);
    ...
    loadChilds = function(viewOid,self,callback){

        var params = {"node":viewOid},
            url = self.treegrid("getSetting", "url") ;
        ...
        $.ajax({
            ...
            success : function(res, textStatus, jqXHR) {
                  var data =  res.data;
                  self.find('td').eq(0).find('i').remove();
                  if(data==null || data.length==0){

                  }
                  else{
                       for(var i = 0;i<data.length;i++){
                           $('#systemConfig').bootstrapTable('insertRow', {
                               index: 0,
                               row: data[i]
                           });
                       }
