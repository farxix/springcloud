...
!function($) {
  ...
  sidebar.prototype.init = function() {
    ...
    _this.$sidebar.on('click', 'li', function(e) {
        e.stopPropagation();
        var $this = $(this);
        _this.resetActive($this);
        if(!$this.find('a:first').hasClass('sidebar-sub-toggle')){
          if($this.parent().hasClass('isParent')){

          }else {
            $('.breadcrumb li:first a').html("当前服务");
          }
            _this.loadUrl($this);
        }else{
          $('.breadcrumb li:first a').html($this.find('a:first').text().substring(2,2+$this.find('a:first').text().length));
        }
    })
  };

  sidebar.prototype.resetActive = function(li) {
      var  _this = this;
      _this.$sidebar.find('li').each(function(){
        if($(this).attr('loadurl')!=undefined){
          $(this).removeClass('nav_li_back')
        }else {
      ...
      if(li.attr('loadurl')!=undefined){
        li.addClass('nav_li_back');
        sessionStorage.setItem('key',li.attr('loadurl'));
      }else {
        li.addClass('active');
      }
  ...
  sidebar.prototype.loadUrl = function(li) {
      var  _this = this;
      $('.container-fluid .title-row').show();
      // $('#mainFrame').css({height:$('#mainFrame').attr('handle-height')});
      $('#mainFrame').attr('src',"./"+li.attr('loadUrl'));
      // $('.page-header h1').html(li.text()+li.attr('displayText'));
      if(li.text().indexOf(' ')>-1){
        $('.breadcrumb .active').html(li.text().substring(2,2+li.text().length));
      }else {
        $('.breadcrumb .active').html(li.text());
      }
